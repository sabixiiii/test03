<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('在其他浏览器和设备上管理和注销您的活动会话。') }}
    </x-slot>

    <x-slot name="content">
        <div class="max-w-xl text-sm text-gray-600">
            {{ __('如有必要，您可以注销所有设备上的所有其他浏览器会话。下面列出了您最近的一些会议；然而，这份清单可能并不详尽。如果您觉得您的帐户已被盗用，您还应该更新您的密码。') }}
        </div>
                            ...
                            <div>
                                <div class="text-xs text-gray-500">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-green-500 font-semibold">{{ __('当前设备') }}</span>
                                    @else
                                        {{ __('最后登录') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <x-jet-dialog-modal wire:model="confirmingLogout">
            ...
            <x-slot name="content">
                {{ __('请输入您的密码以确认您想退出所有设备上的其他浏览器会话。') }}
